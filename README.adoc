...
+
....
$ cf set-env message-generation CF_TARGET https://api.wise.com
Setting env variable 'CF_TARGET' to 'https://api.wise.com' for app message-generation in org myorg / space outer as user...
OK
...
Setting env variable 'CF_TARGET' to 'https://api.wise.com' for app greeter in org myorg / space outer as user...
OK
TIP: Use 'cf restage' to ensure your env variable changes take effect
$ cf restage greeter
....
...
Connected, tailing logs for app message-generation in org myorg / space outer as user...
...
. To get a different greeting message, you can provide `salutation` and `name` parameters, as in `[ROUTE]/hello?salutation=Come%20on&name=Jesse`. The Greeter application will send those parameters to the Message Generation application and the resulting greeting will be customized to match.
+
